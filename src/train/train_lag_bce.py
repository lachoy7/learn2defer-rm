...
def train_router_full_defer(
    ...
    print(
        f"Training Learn-to-Defer (BCE primal){suffix} | target_escal={target_escalation:.1%} | "
        f"λ0={lambda_init} | λ_lr={lambda_lr} | stage1_index={stage1_index}"
    )
    ...
    wb = WandbLogger(
        ...
        entity=wandb_entity,
        run_name=wandb_run_name or f"lag_bce_target{target_escalation:.2f}_idx{stage1_index}_{input_mode}",
        group=wandb_group or f"lag_bce_{input_mode}",
        tags=wandb_tags,
        trainer="lag_bce",
        config={
            "trainer": "lag_bce",
    ...
    for epoch in tqdm(range(epochs), desc="Training"):
        ...
        for b in train_loader:
            ...
            loss = bce + float(lambda_cost) * s.mean()
            ...
            # Dual ascent step after every batch (was: after every epoch)
            # If escalation is above budget, increase lambda; if below, decrease.
            lambda_cost = max(0.0, min(lambda_max, lambda_cost + lambda_lr * (batch_escal - target_escalation)))
...
# Target escalation values to sweep over (run with: modal run scripts/train_lag_bce.py)
TARGET_ESCALATION_SWEEP = [0.25] #[0.05, 0.1, 0.15, 0.2, 0.25]

# =============================================================================
# HYPERPARAMETERS — edit here or override via CLI (--lambda-lr 0.1 etc.)
# =============================================================================
HYPERPARAMS = {
    "stage1_index": 0,
    "lambda_init": 0.0,
    "lambda_lr": 0.005,        # dual learning rate (Lagrange multiplier step size)
    "lambda_max": 100.0,
    "epochs": 100,
    "lr": 1e-3,               # model AdamW learning rate
    "patience": 100,
    "temperature": 1.0,
}
...
def _parse_args():
    """Parse CLI args for local runs. Overrides HYPERPARAMS when provided."""
    p = argparse.ArgumentParser(description="Train Learn-to-Defer router (Lagrangian BCE)")
    p.add_argument("--stage1-index", type=int, default=HYPERPARAMS["stage1_index"])
    p.add_argument("--lambda-init", type=float, default=HYPERPARAMS["lambda_init"])
    p.add_argument("--lambda-lr", type=float, default=HYPERPARAMS["lambda_lr"],
                   help="dual learning rate (Lagrange multiplier step size)")
    p.add_argument("--lambda-max", type=float, default=HYPERPARAMS["lambda_max"])
    p.add_argument("--epochs", type=int, default=HYPERPARAMS["epochs"])
    p.add_argument("--patience", type=int, default=HYPERPARAMS["patience"])
    p.add_argument("--temperature", type=float, default=HYPERPARAMS["temperature"])
...
if __name__ == "__main__":
    # =============================================================================
    # FINAL TRAINING CALL (local) — edit HYPERPARAMS above or pass --lambda-lr 0.1 etc.
    # =============================================================================
    args = _parse_args()
...
# =============================================================================
# MODAL GPU TRAINING — run with: modal run scripts/train_lag_bce.py (from repo root)
# =============================================================================
try:
    import modal
except ImportError:
...
if modal is not None:
    L2D_MODAL_APP = modal.App("l2d-rm-train-lag-bce")
    L2D_IMAGE = (
    ...
    L2D_VOLUME_FULL = modal.Volume.from_name("l2d-rm-checkpoints-bcelag-full", create_if_missing=True)
    L2D_VOLUME_EMBEDDING_ONLY = modal.Volume.from_name("l2d-rm-checkpoints-bcelag-embeddingonly", create_if_missing=True)
    L2D_VOLUME_SCALAR_ONLY = modal.Volume.from_name("l2d-rm-checkpoints-bcelag-scalaronly", create_if_missing=True)
    ...
    def train_on_modal(
        stage1_index: int = HYPERPARAMS["stage1_index"],
        lambda_init: float = HYPERPARAMS["lambda_init"],
        lambda_lr: float = HYPERPARAMS["lambda_lr"],
        lambda_max: float = HYPERPARAMS["lambda_max"],
        ...
        import train.train_lag_bce as train_mod
        ...
        vol.commit()
        vol_name = f"l2d-rm-checkpoints-bcelag-{input_mode}"
